...
class Elasticity:
    def __init__(self, mesh, Vh, prior):
        ...
        mtrue_exp = dl.Expression('0.2*(15.0 - 5.0*sin(3.1416*(x[1]/20.0 - 0.5)))',
        ...
        self.u_bdr = dl.Expression(("0.0","0.0","0.0"), element=Vh[STATE].ufl_element()) 
        self.u_bdr0 = dl.Expression(("0.0","0.0","0.0"), element=Vh[STATE].ufl_element()) 
...
#        print ( "||m||", x[PARAMETER].norm("l2"), "||u||", x[STATE].norm("l2"), "||C||", C.norm("linf") )
    ...
    def computeObservation(self, u_o):
        ...
        #save data
        u_tr=vector2Function(u_o, self.Vh[STATE])
        dl.File("results/u_true.pvd") << u_tr
        
        # Create noisy data, ud
        MAX = u_o.norm("linf")
        parRandom.normal_perturb(.01 * MAX, u_o)
        u_ob=vector2Function(u_o, self.Vh[STATE])
        dl.File("results/u_obs.pvd") << u_ob
...
if __name__ == "__main__":
 ...
 #   modelVerify(model, m0.vector(), is_quadratic = False, verbose = (rank==0))
    ...
    atrue = dl.interpolate(dl.Expression('0.2*(15.0 - 5.0*sin(3.1416*(x[1]/20.0 - 0.5)))',element=Vh[PARAMETER].ufl_element()), Vh[PARAMETER])
